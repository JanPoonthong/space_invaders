...
IMG_NAMES = ['ship', 'laser', 'explosionpurple', 'explosioncyan',
             'explosiongreen', 'mystery', 'enemy1_2', 'enemy2_1', 'enemy3_1']
IMAGES = {
    name: pygame.image.load(IMAGE_PATH + '{}.png'.format(name)).convert_alpha()
    for name in IMG_NAMES}

SOUND_NAMES = ['background', 'shoot', 'invaderkilled', 'mysteryentered',
               'mysterykilled']
SOUNDS = {name: pygame.mixer.Sound(SOUND_PATH + '{}.wav'.format(name))
...
MYSTERY_IMG = pygame.transform.scale(IMAGES['mystery'].convert_alpha(),
                                     (90, 40))
...
class Enemies:
    ...
    def __init__(self):
        ...
        self.cyan_rect = [[], []]
        self.green_rect = [[], []]
        self.rects = [self.purple_rect, self.cyan_rect[0], self.cyan_rect[1],
                      self.green_rect[0], self.green_rect[1]]
    ...
    def enemies_generator(self):
        ...
        for enemy in range(self.num_in_a_row):
            ...
            images = [self.p_img[enemy], self.c_img[enemy], self.c_img[enemy],
                      self.g_img[enemy], self.g_img[enemy]]
            hits = [self.purple_hit, self.cyan_hit[0], self.cyan_hit[1],
                    self.green_hit[0], self.green_hit[1]]
    ...
    def enemies_collision(self):
        ...
        self.hide_enemies(self.green_rect[0], EXPLOSION_GREEN,
                          self.green_hit[0])
        self.hide_enemies(self.green_rect[1], EXPLOSION_GREEN,
                          self.green_hit[1])

    def is_enemy_hit_ship(self):
        for i in range(self.num_in_a_row):
            if (self.purple_rect[i].y >= 410 and self.purple_hit[i] is not True
                    or self.cyan_rect[0][i].y >= 410 and self.cyan_hit[0][
                        i] is not True
                    or self.cyan_rect[1][i].y >= 410 and self.cyan_hit[1][
                        i] is not True
                    or self.green_rect[0][i].y >= 410 and self.green_hit[0][
                        i] is not True
                    or self.green_rect[1][i].y >= 410 and self.green_hit[1][
                        i] is not True):
                return True
        return False
...
class Explosion:
    ...
    def update(self):
        if self.counter > 0:
            SCREEN.blit(self.explosion_color,
                        (self.enemy_rect_x, self.enemy_rect_y))
            self.counter -= 1
        elif self.counter == 0:
...
class Bunker:
    ...
    def draw_bunker(row_empty, row_bunker, x_axis, y):
        for row in range(9):
            ...
            row_bunker.append(pygame.draw.rect(SCREEN, GREEN,
                                               (first_x, y, 10, 10)))
    ...
    def all_bunker(self):
        ...
        for i in range(4):
            ...
            bunker_y = [70, 350, 650]
            for k in range(3):
                self.draw_bunker(bunker[k].bunker_empty[i],
    ...
    def bunker_collision(self):
        for i in range(4):
            for j in range(3):
                self.hide_rectangle_pixel(bunker[j].bunker_row[i],
                                          bunker[j].bunker_empty[i])
                self.hide_rectangle_pixel(bunker[j].bunker_row[i],
                                          bunker[j].bunker_empty[i])
                self.hide_rectangle_pixel(bunker[j].bunker_row[i],
                                          bunker[j].bunker_empty[i])
...
class GameOver:
    ...
    def draw_next_round():
        ...
        game_over_text = TITLE_FONT.render('Next Round Coming soon!', True,
                                           WHITE)
